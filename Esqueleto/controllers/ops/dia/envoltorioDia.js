 ...
 */
module.exports = {
    envoltorioDia :function envoltorioDia(campo, funcion, respuesta, filtro={})
    {
        ...
        ModeloClima
            ...
            .find(filtro, campo, {sort: {dia: 1}}, function(err,datos)
            {
                ...
                clasificarDatosDia(datos, dataDict);
                ...
                rellenarMensajeRespuestaDia(resMsg, resDict, campo, filtro, funcion);
                //Envio respuesta
 ...
 */
function clasificarDatosDia(datos, dataDict)
{
    datos.forEach(element =>
    {
        //Obtengo la fecha (día del mes) de la entrada actual.
        var diaActual = element['dia'].getDate();
        console.log("Dia: " + diaActual);
        //Si no hay datos todavia para ese día creo lista
        if(dataDict[diaActual]==undefined)
        {
            dataDict[diaActual] = [];
        }
        //añado datos de la entrada actual a la lista
        //para el día en el que estan registrados los datos
        dataDict[diaActual].push(element);
    });
}
 ...
 */
function rellenarMensajeRespuestaDia(resMsg, resDict, campo, filtro, funcion)
{
    resMsg['label'] = [];
    resMsg['data'] = [];
