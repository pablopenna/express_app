 ...
 * 'probLluvia'.
 */
function lameinnerMediaGeometricaLluvia(datos)
{
    //datos es un array con los objetos retornados por find
    //res.send(datos);
    //MEDIA ARMONICA = (elem1*elem2*...*elemN)^(1/N)
    var producto = 1;
    var numElementos = 0;
    datos.forEach( element => {
        console.log("ELEMENTO[lluvia]: " + element['probLluvia']);
        ...
        if(element['probLluvia'] != 0)
        {
            producto *= element['probLluvia'];
            numElementos++;
            console.log("PROD.GEO: " + producto);
        }
    });
    console.log("LONG: " + numElementos);
    console.log("MUL: " + producto);
    var media = Math.pow(producto, 1/numElementos);
    //meto la media en un json
